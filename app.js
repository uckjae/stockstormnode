...
const insertPriceToKafka = require('./modules/InsertPriceToKafka')

symbols.find({}).select('-_id symbol').then((symbolList) => {
    for (let i = 0; i < symbolList.length; i++) {
        const j = schedule.scheduleJob('*/5 * * * * *', function () {
          console.log('kafkapub '+symbolList[i].symbol)
          insertPriceToKafka.insertPriceToKafka(symbolList[i].symbol);
        });
    }
});
